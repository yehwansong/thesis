...
OFFSET=$((RANDOM % 50))
echo "Random offset: $OFFSET (different images each run)"
echo ""
...
for QUERY in "${QUERIES[@]}"; do
  ...
  SEARCH=$(curl -s "https://en.wikipedia.org/w/api.php?action=query&list=search&srsearch=${QUERY}&srnamespace=6&srlimit=6&sroffset=${OFFSET}&format=json" -H "User-Agent: SunImageCollector/1.0")
